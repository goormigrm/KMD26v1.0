...
import { MatchSim } from "./kernel.js?v=88748df34e";
...
/* 초보자용 프리셋 — 슬라이더 여덟 개도 부담인데 조건까지 짜라면 아무도 안 씁니다.
   ...
   ⚠ 한 칸은 **한 번만** 발동합니다(checkOrders 의 _ordDone). 그래서 칸을 채울 때는
     "먼저 걸릴 조건"을 위에 두는 것이 아니라, **서로 다른 상황**을 담아야 합니다.
     예: `지고 있으면`(6번)은 전반 5분에도 걸리므로, 후반 승부수는 `60분에 지고 있으면`
     (9번)처럼 시각이 붙은 조건으로 잡습니다.
   ⚠ 조건이 볼 수 있는 것은 **분 · 득실차 · 내 퇴장 수** 셋뿐입니다.
     "상대가 압박을 올리면" 같은 건 아직 못 봅니다.

   조건 번호 — 1:15분 2:30분 3:전반끝 4:60분 5:75분 6:지고 7:이기고 8:비기고
               9:60분+지고 10:60분+이기고 11:75분+지고 12:75분+이기고
               13:두 골 앞서면 14:두 골 뒤지면 15:퇴장
   행동 번호 — 1:총공세 2:공격적 3:잠근다 4:실리로 5:압박↑ 6:압박↓ 7:템포↑ 8:템포↓
               9:길게 10:짧게 11:측면 12:중앙 13:거칠게 14:카드아낌 15:역습 */
export const ORDER_PRESETS = {
  ...
  // ── 상황별 ──────────────────────────────────────────────
  /* 전반은 체력을 아끼고 후반에 쏟아붓는다. 압박을 내려 두었다가 60분에 되올린다. */
  "후반에 승부": [packOrder(1, 6), packOrder(4, 5), packOrder(9, 2), packOrder(11, 1), 0, 0],
  /* 앞서면 곧바로 문을 닫는다 — 리드를 지키는 것이 목적일 때 */
  "리드 지키기": [packOrder(7, 4), packOrder(10, 3), packOrder(13, 8), packOrder(12, 14), 0, 0],
  /* 비기고 있으면 서서히 올리고, 후반에도 그대로면 총공세 */
  "무승부는 싫다": [packOrder(2, 2), packOrder(8, 5), packOrder(11, 1), packOrder(14, 9), 0, 0],
  /* 두 골 이상 뒤졌을 때만 크게 던진다. 그 전에는 형태를 지킨다 */
  "따라잡기": [packOrder(14, 1), packOrder(9, 2), packOrder(5, 9), packOrder(5, 11), 0, 0],
  /* 열 명이 되면 즉시 내려앉고(잠근다가 역습까지 켠다) 두 번째 퇴장을 막는다 */
  "퇴장 대비": [packOrder(15, 3), packOrder(15, 14), packOrder(11, 2), 0, 0, 0],

  // ── 형태를 바꾼다 ───────────────────────────────────────
  /* 짧게 돌리며 압박하다가, 안 풀리면 길게 넘긴다 */
  "티키타카 → 롱볼": [packOrder(1, 10), packOrder(3, 5), packOrder(9, 9), packOrder(11, 1), 0, 0],
  /* 낮게 서서 받아치다가 지고 있으면 라인을 올린다 */
  "웅크렸다 역습": [packOrder(1, 15), packOrder(2, 6), packOrder(9, 2), packOrder(11, 1), 0, 0],
  /* 측면으로 벌려 크로스, 후반에는 중앙으로 좁혀 밀집 */
  "측면에서 중앙으로": [packOrder(1, 11), packOrder(4, 12), packOrder(9, 2), 0, 0, 0],
  /* 카드를 아끼다가 지고 있을 때만 거칠게 — 퇴장이 나면 바로 되돌린다 */
  "카드 관리": [packOrder(1, 14), packOrder(11, 13), packOrder(15, 14), packOrder(15, 4), 0, 0],

  // ── 체력 ────────────────────────────────────────────────
  /* 압박과 템포를 아꼈다가 마지막 15분에 몰아친다 */
  "막판 스퍼트": [packOrder(1, 6), packOrder(1, 8), packOrder(5, 5), packOrder(5, 7), packOrder(11, 1), 0],
};
